...
#	define LMPLATFORM_WINDOWS 1
#	ifndef _WIN64
#		error "64-bit is required on Windows!"
#	endif
#elif defined(__linux__) || defined(__gnu_linux__)
// Linux OS
#	define LMPLATFORM_LINUX 1
#	if defined(__ANDROID__)
#		define LMPLATFORM_ANDROID 1
#	endif
#elif defined(__unix__)
// Catch anything not caught by the above.
#	define LMPLATFORM_UNIX 1
#elif defined(_POSIX_VERSION)
// Posix
#	define LMPLATFORM_POSIX 1
#elif __APPLE__
// Apple platforms
#	define LMPLATFORM_APPLE 1
#	include <TargetConditionals.h>
#	if TARGET_IPHONE_SIMULATOR
// iOS Simulator
#		define LMPLATFORM_IOS 1
#		define LMPLATFORM_IOS_SIMULATOR 1
...
// Exports
#	ifdef _MSC_VER
#		define LMAPI __declspec(dllexport)
#	else
#		define LMAPI __attribute__((visibility("default")))
#	endif
#else
// Imports
#	ifdef _MSC_VER
/** @brief Import/export qualifier */
#		define LMAPI __declspec(dllimport)
#	else
/** @brief Import/export qualifier */
#		define LMAPI
#	endif
 ...
 */
#define LMCLAMP(value, min, max) ((value <= min) ? min : (value >= max) ? max \
																	   : value)

// Inlining
#if defined(__clang__) || defined(__gcc__)
/** @brief Inline qualifier */
#	define LMINLINE __attribute__((always_inline)) inline

/** @brief No-inline qualifier */
#	define LMNOINLINE __attribute__((noinline))
#elif defined(_MSC_VER)

/** @brief Inline qualifier */
#	define LMINLINE __forceinline

/** @brief No-inline qualifier */
#	define LMNOINLINE __declspec(noinline)
#else

/** @brief Inline qualifier */
#	define LMINLINE static inline
...
#define LMIBIBYTES(amount) ((amount) * 1024ULL)
...
#define LMILOBYTES(amount) ((amount) * 1000ULL)

LMINLINE u64 get_aligned(u64 operand, u64 granularity) {
	return ((operand + (granularity - 1)) & ~(granularity - 1));
}

LMINLINE LMrange get_aligned_range(u64 offset, u64 size, u64 granularity) {
	return (LMrange){get_aligned(offset, granularity), get_aligned(size, granularity)};
}

#define LMSWAP(type, a, b) \
	{                     \
		type temp = a;    \
		a = b;            \
		b = temp;         \
...
#define LMMIN(x, y) (x < y ? x : y)
#define LMMAX(x, y) (x > y ? x : y)
